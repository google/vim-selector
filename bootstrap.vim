...
let s:selector_path = expand('<sfile>:p:h')
...
if !exists('*maktaba#compatibility#Disable')
  try
    ...
    let s:guess1 = fnamemodify(s:selector_path, ':h') . s:slash . 'maktaba'
    let s:guess2 = fnamemodify(s:selector_path, ':h') . s:slash . 'vim-maktaba'
    ...
    try
      ...
      echohl ErrorMsg
      echomsg 'Maktaba not found, but selector requires it. Please either:'
...
call maktaba#plugin#GetOrInstall(s:selector_path)
